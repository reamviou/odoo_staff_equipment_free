...
# from .globals import lic_mod

from datetime import datetime, timedelta, time
import pytz


class VitouEquFunc(models.Model):
     _name = 'vitouequ.func'
     ...
     # def is_my_saft(self):
     #      info = self.env[lic_mod].lic_info
     #      issaft = self.env[lic_mod].lic_ok()
     #      if not issaft :
     ...
     def get_prefix(self, words):
          # s = "Python makes life easier"
          initials = [word[0] for word in words.split() if word]
          # print(initials)  # ['P', 'm', 'l', 'e']
          w = initials
          r = "".join(w)
          # print(r.upper())  # 'Pmle'
          return r.upper()
